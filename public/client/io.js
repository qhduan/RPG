...
(function () {
  ...
  function DamageHandle(data) {
    for (var key in data) {
      if (Game.area.actors[key]) Game.area.actors[key].damage(data[key]);
      if (Game.area.heros[key]) Game.area.heros[key].damage(data[key]);
    }
  }
    ...
    Game.io.socket.on("connect", function () {
      ...
      socket.on("move", function (data) {
        if (Game.area.heros[data.id]) {
          Game.area.heros[data.id].gotoXY(data.data.x, data.data.y, data.data.speed, false);
        }
      });
      ...
      socket.on("attack", function (data) {
        if (Game.area.heros[data.id]) {
          Game.area.heros[data.id].fire(data.data.num, data.data.direction);
        }
      });
