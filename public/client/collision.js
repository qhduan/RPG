...
(function () {
  ...
  function rectngleCollide(spriteA, spriteB, rectA, rectB) {
    ...
    var data = {
      x1: spriteA.x - rectA.regX, // A的左上角x坐标
      y1: spriteA.y - rectA.regY, // A的左上角y坐标
      x2: spriteB.x - rectB.regX, // B的左上角x坐标
      y2: spriteB.y - rectB.regY, // B的左上角y坐标
      w1: rectA.rect.width, // A的宽
      h1: rectA.rect.height, // A的高
      w2: rectB.rect.width, // B的宽
      h2: rectB.rect.height, // B的高
      x3: spriteA.x - rectA.regX + rectA.rect.width, // A的右上角
      y3: spriteA.y - rectA.regY + rectA.rect.height, // A的左下角
      x4: spriteB.x - rectB.regX + rectB.rect.width, // B的右上角
      y4: spriteB.y - rectB.regY + rectB.rect.height // B的左下角
    };
    ...
    if (data.width < rectA.rect.width + rectB.rect.width && data.height < rectA.rect.height + rectB.rect.height) return data;
    return false;
  }
  ...
  function pixelCollide(spriteA, spriteB, rectA, rectB, data, deltaX, deltaY, deltaW, deltaH) {
    ...
    context.drawImage(rectA.image, rectA.rect.x, rectA.rect.y, data.w1, data.h1, data.x1, data.y1, data.w1, data.h1);

    var pixelOld = context.getImageData(data.x1 + deltaX, data.y1 + deltaY, data.w1 + deltaW, data.h1 + deltaH).data;

    // draw spriteB
    context.drawImage(rectB.image, rectB.rect.x, rectB.rect.y, data.w2, data.h2, data.x2, data.y2, data.w2, data.h2);
  ...
  // 技能碰撞检测
  Game.spellCollision = function (spellSprite, actorSprite) {
    // 角色只检测frame 0，因为角色老变动，避免卡住，只检测第一个frame
    var spellRect = spellSprite.spriteSheet.getFrame(spellSprite.currentFrame);
    //console.log(spellSprite.currentFrame);
    // 阻挡的块则检测当前frame，因为地块一般无变化
    var actorRect = actorSprite.spriteSheet.getFrame(actorSprite.currentFrame);

    var data = rectngleCollide(spellSprite, actorSprite, spellRect, actorRect);
    if (data) {
      //return true;
      // 和角色碰撞检测对比，技能碰撞检测无delta
      return pixelCollide(spellSprite, actorSprite, spellRect, actorRect, data);
    }
    return false;
  };
})();
