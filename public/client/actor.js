...
(function () {
  ...
  /*
    ...
    属性：
      ...
      this.sprite 精灵
  */
  Game.ActorClass = (function (_Game$EventClass) {
    function ActorClass(actorData) {
      ...
      this.hpbarBox.graphics.beginStroke("black").drawRect(0, 0, 30, 3);
      ...
      this.mpbarBox.graphics.beginStroke("black").drawRect(0, 0, 30, 3);
      ...
      // 根据血量不同设定不同颜色
      var hpcolor = "green";
      if (this.currentHitpoint / this.data.hitpoint < 0.25) hpcolor = "red";else if (this.currentHitpoint / this.data.hitpoint < 0.5) hpcolor = "yellow";

      this.hpbar.graphics.clear().beginFill(hpcolor).drawRect(0, 0, parseInt(this.currentHitpoint / this.data.hitpoint * 30), 3);
      ...
      this.mpbar.x = 0;
      this.mpbar.y = 14;
      this.mpbar.graphics.clear().beginFill("blue").drawRect(0, 0, parseInt(this.currentManapoint / this.data.manapoint * 30), 3);
      ...
      // 发送完成事件，第二个参数代表一次性事件
      _get(Object.getPrototypeOf(ActorClass.prototype), "emit", this).call(this, "complete", true);
    }

    _inherits(ActorClass, _Game$EventClass);
    ...
    _createClass(ActorClass, [{
      ...
      value: function use() {
        if (this.data.use) {
          if (this.data.use.type == "talk") {
            ...
            dialogueBox.graphics.beginStroke("black").beginFill("white").drawRoundRect(0, 0, w + 10, h + 10, 5);

            dialogueBox.graphics.beginFill("white").moveTo(w + 5, h + 8).lineTo(w, h + 15).lineTo(w - 5, h + 8).beginStroke("white").lineTo(w + 5, h + 8);
      ...
      value: function decreaseHitpoint(hp) {
        ...
        if (this.currentHitpoint <= 0) {
          ...
          Game.items.item0001.clone(function (dead) {
            dead.draw(Game.itemLayer, _this.sprite.x, _this.sprite.y);
          });
      ...
      value: function damage(hp, mp) {
        ...
        if (typeof hp == "number" && !isNaN(hp) && hp > 0) this.decreaseHitpoint(hp);
      ...
      value: function fire(num, direction) {
        ...
        this.data.spells[spell].fire(self, "attack" + direction, function (hittedActorIds) {
          _this2.attacking = false;
          if (hittedActorIds && hittedActorIds.length) {
            Game.io.hit(_this2.data.spells[spell].id, hittedActorIds);
          }
        });
      ...
      key: "gotoXY",
      value: function gotoXY(x, y, speed, collisionTest, callback) {
        var _this3 = this;
        ...
        this.gotoXYListener = createjs.Ticker.on("tick", function () {
          return toXY();
        });
        ...
        var toXY = function toXY() {
          ...
          if (x == 0 && y == 0) {
            if (_this3.gotoXYListener) {
              createjs.Ticker.off("tick", _this3.gotoXYListener);
              _this3.gotoXYListener = null;
            }
            _this3.stop();
            ...
            _this3.go(state, direction, skew, collisionTest);
          } else if (Math.abs(x) > limit) {
            if (x > 0) {
              _this3.go(state, "right", speed, collisionTest);
              x -= speed;
            } else {
              _this3.go(state, "left", speed, collisionTest);
              x += speed;
            }
          } else if (Math.abs(y) > limit) {
            if (y > 0) {
              _this3.go(state, "down", speed, collisionTest);
              y -= speed;
            } else {
              _this3.go(state, "up", speed, collisionTest);
              y += speed;
            }
          } else if (Math.abs(y) != 0) {
            if (y > 0) {
              _this3.go(state, "down", y, collisionTest);
              y = 0;
            } else {
              _this3.go(state, "up", -y, collisionTest);
              y = 0;
            }
          } else if (Math.abs(x) != 0) {
            if (x > 0) {
              _this3.go(state, "right", x, collisionTest);
              x = 0;
            } else {
              _this3.go(state, "left", -x, collisionTest);
              x = 0;
            }
          }
        };
      ...
      value: function CheckDirection(direction, step, collisionTest) {
        ...
        var CheckCollision = function CheckCollision(t) {
          ...
          if (Game.area.map.blockedMap[t.y] && Game.area.map.blockedMap[t.y][t.x]) {
            if (Game.actorCollision(_this4.sprite, Game.area.map.blockedMap[t.y][t.x])) {
              tested[i] = true;
              return true;
            }
          }
